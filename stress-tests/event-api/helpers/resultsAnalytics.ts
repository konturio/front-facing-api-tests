...
const buildAnalyticsMessage = (resultsAnalytics: ResultsAnalysis): string => {
  ...
  return `${testSummary} /n ${perf} | ${statusText} | ${errors} | ${longReqs} | ${ids} | ${urls} | ${sizes} 😎. * - N/A means that the test did not use this parameter.`;
};
